...
#### 待办：补一套公开基准，让数字有外部可比性

- [ ] **VoiceBank+DEMAND**（约 1 GB）——增强领域的事实标准测试集
- [ ] 本地 PESQ 闭环（当前受 I-05 阻塞，只能在 Colab 侧算）
- [ ] CSIG / CBAK / COVL（与 PESQ 同源的复合指标，文献必报）

**为什么需要**：现在三套测试集全部是**自己合成的**。分层可控是优点
（SNR/噪声/RT60 都精确标定，见 I-24/I-32 修完后实测偏差 0.0000 dB），
但**没有任何外部可比性**——说"specsub 提升 0.42 dB"，没人知道这个数字
算好算坏，因为没有第二个人在同一批数据上报过。

F-14 已经用第三方实现（logmmse / noisereduce）做过横向对照，
证明了本项目的 DSP 基线不弱；但那是**在我们自己的数据上比别人的代码**，
还差另一半：**在公认的数据上跑我们的代码**。两者合起来才完整。

VoiceBank+DEMAND 是性价比最高的选择：体积小、几乎所有增强论文都报它
（SEGAN / MetricGAN / DEMUCS 等），补上之后指标能直接和文献对齐。

**成本不小，不是顺手能做的**：约 1 GB 下载 + 一套新的数据加载与
`index.json` 适配 + PESQ 依赖问题。要单独排期。
