...
### 2026-08-07（续）— 真实 CER 落地，发现全项目最重要的结论

- **测试集重新生成并端到端验证**（I-21 完全解决）：用户在 Colab 上重跑修复后的
  `01_data_prep.ipynb`，下载新 `testset.zip`。同源样本前后对比 CER 从 0.519
  降到 0.140（降 73%），确认修复生效
- **过程中又踩出两个新 bug**：
  - 截断校验本身查错了信号（先是查混响后的参考音频，被混响拖尾污染；
    再是 VAD 的 hangover 跟正常收尾静音混淆）——最终改成纯代码逻辑的
    结构性断言，不再依赖任何声学假设（I-21 的第二、三次修正）
  - `make_rir()` 镜像阶数写死成 12，导致 T60 超过约 0.6 秒后完全失效，
    直接解释了 F-06 里"T60=0.9 反而比 0.6 好"的悬案——两者其实是同一个
    混响强度。已修复（自适应阶数 + 向量化 + 分块），测试集 T60 扫描
    需要重新生成才有效（I-22）
- **实现 `rtse-eval` CLI**：补上 `pyproject.toml` 声明已久但一直没写的入口点，
  支持断点续跑（每个方法跑完立刻落盘）——第一版没做这个，后台任务被中断后
  50 分钟的计算全部丢失，加上续跑能力后验证过第二次调用能正确跳过已完成部分
- **首次拿到真实 CER 数字**：全量 780 样本客观指标 + 78 样本分层抽样 CER。
  **核心发现（F-08）：全部 6 种降噪方法的 CER 都比不处理更差**，
  跟 SI-SDR 方向完全相反——这是本项目"降噪对 ASR 有没有用"这个核心问题
  第一次有真实数据支撑的答案，方向出乎意料，是目前最重要的发现，
  仍需全量复核确认结论稳固
- **准备好换机器继续**：整理 GitHub 仓库时发现 `.gitignore` 一个缺前导斜杠的
  疏漏，把 `src/rtse/data/`、`src/rtse/models/`（含 CRN 模型定义）整个排除在
  仓库之外——**GitHub 上的代码此前根本 clone 不下来跑**（I-23）。已修复并用
  真实 clone 验证；训练好的三档 ONNX 模型（9.5 MB）随仓库分发；新增
  `docs/SETUP_NEW_MACHINE.md` 和 `rtse-enhance` CLI（同样是声明未实现的缺口）

### 2026-08-07（续）— 数据集重新设计，v1/v2 废弃

用户重新规划了数据方案，理由是四个维度：声学真实性、实验可控性、算力性价比、
面试评估说服力。**旧的两版（THCHS-30+MUSAN、THCHS-30+DNS噪声）全部删除**，
连同它们测出的所有指标与实验发现一起清空——那些结论在新数据上不成立，
留着只会造成混淆（旧内容在 git 历史 `ad69d0b` 里）。

- **新数据设计**（详见 [`notebooks/README.md`](../notebooks/README.md)）：
  - 训练干净语音：**DNS Challenge 4 `read_speech`**，1 分片 ≈ 21 小时。
    选 DNS4 而不是 DNS5 是因为 **DNS5 的 clean speech 是 `split` 切片
    （`partaa`~`partau`），必须全部下载才能拼接解压**，做不了小规模子集；
    DNS4 的分片是独立 `.tar.bz2`，可以只下需要的几片
  - 训练噪声：DNS `noise_fullband`（AudioSet + Freesound）3 分片
  - RIR：DNS 真实 IR **+ 本项目合成**，测试集里并行分层
    （合成的做受控 RT60 扫描，真实的验泛化，避免"仿真过拟合"）
  - 中文 ASR 评测：**WenetSpeech `test_meeting`**
- **核心设计：训练用英文、评测用中文**。前端在 DNS（英文为主）上训练，
  直接在中文 WenetSpeech 上测 CER——CER 若仍改善，就证明模型学到的是
  纯声学去噪规律而非语言相关模式。同语种自训自测拿不到这个论证
- **WenetSpeech 获取方式实测**：官方（wenet.org.cn）要填 Google 表单拿密码，
  没法脚本化；HuggingFace 的 `lmms-lab/WenetSpeech` 镜像**不需要登录**，
  `test_meeting` 音频+中文转写打包在一个 220 MB parquet 里，soundfile 能直接解 opus。
  ⚠️ 该镜像的 `test_net` 只有 124 个文件（残缺），不可用
- **新增噪声平稳性判别**（[`rtse/dsp/stationarity.py`](../src/rtse/dsp/stationarity.py)）：
  DNS 噪声不带平稳性标注，而"DSP 在稳态噪声上够用、在非稳态上失效"这条
  核心对照完全依赖这个分组。实现是三步迭代出来的，每步都在修上一步暴露的问题：
  ① 时间轴平滑（压掉周期图估计方差，否则白噪声被判成非稳态）；
  ② **帧能量去趋势**（否则车载噪声那种 0.2 Hz 慢调制拿到 16.7 dB 动态范围被误判，
  但 MCRA 对慢变化跟得毫无压力）；③ 取 P95−P05 而非最大最小值差。
  在 7 类构造上已知平稳性的合成噪声上**分类全部正确**，12 项测试锁定
- **记录一个否定结果**：最初设计里还有"谱通量"作为第二判据，
  实测发现白噪声的谱通量（0.140）反而**高于** babble（0.096）——
  周期图方差在归一化后依然主导帧间差。降级成诊断量，不参与判决，
  并写了一条测试把这个事实钉死，防止后人重走这条路
- 测试集分层：SNR{−5,0,5,10,15} × 噪声{稳态,非稳态} × RIR{合成 4 档 RT60, 真实}
