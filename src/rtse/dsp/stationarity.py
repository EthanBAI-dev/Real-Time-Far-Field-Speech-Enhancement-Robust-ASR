...
已知边界情况：极低频主导 + 慢调制的噪声
----------------------------------------
`car` 型噪声（1/f^2.5 的陡峭低频 + 0.2 Hz 幅度调制）**判决不稳定**：
12 个随机种子实测动态范围在 0.67~16.22 dB 之间，横跨门限。
根因是这类信号的能量集中在少数极低频分量上，帧能量本身就是个高方差的
慢变随机过程，1 秒的中值去趋势去不干净。

试过限带到语音频段（250~3800 Hz）来规避——**没用，反而更糟**：
car 的跨度只降到 7.25 dB，同时把 babble/cafeteria 从 10~12 dB 压到 3~6 dB，
两者范围直接重叠，整体分离度下降。所以保持全带度量。

**这个限制是可接受的**，因为本模块的用途是给几万个真实噪声文件做**分组**，
不是给单个文件下权威判决：稳定的风扇声会可靠地落进稳态组，关门/键盘会可靠地
落进非稳态组，边界样本落到哪一边有一定随意性但不影响两组的整体性质。
notebook 里会打印两组的数量，比例悬殊时应当调门限。
"""
...
#: **这个值是在本项目合成噪声上标定的**（每类跑 12 个随机种子取范围）：
#:
#:   white 0.49~0.65 / hum 0.04~0.05 / pink 6.19~8.16   → 稳定判为稳态
#:   babble 10.1~12.6 / cafeteria 10.0~11.8 / keyboard 319~340 → 稳定判为非稳态
#:   car 0.67~16.22 → **横跨门限，判决随随机实现翻转**
#:
#: 真实录音的分布更宽更连续，这个门限**需要在真实噪声上重新标定**，
#: 所以它是参数不是常数。
