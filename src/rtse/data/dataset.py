...
class MixConfig:
    ...
    snr_range: tuple[float, float] = (-5.0, 35.0)
    """加噪时的 SNR 采样区间。

    **上限必须显著高于"需要处理"的范围。** 原来是 20 dB，模型因此
    **从未见过接近无噪的输入**，学到的映射变成"无论看到什么都往
    录音棚级干净去推"——在训练分布内是最优策略，拿到分布外就是
    无条件过抑制：实测对干净输入下重手，SI-SDR 从 128 dB 打到 7.27 dB，
    并直接导致下游 CER 在高 SNR 段大幅恶化（见 docs/FINDINGS.md F-11）。
